...
function DownloadResult({ data }) {
    return (
        <div>
            ...
            {data.comment_attachment_items?.length > 0 && (
                <Section title="💬 댓글 첨부파일">
                    <CommentAttachmentResult items={data.comment_attachment_items} />
                </Section>
            )}
        </div>
    );
...
function CommentAttachmentResult({ items }) {
    const totalFiles = items.reduce((sum, item) => {
        const paths = item.sub_elements?.['#commentAttachLink']?.[0]?.download_path;
        return sum + (Array.isArray(paths) ? paths.length : paths ? 1 : 0);
    }, 0);

    return (
        <div>
            <div className="download-summary">
                첨부 있는 댓글 {items.length}개 / 다운로드 파일 {totalFiles}개
            </div>
            <div className="ca-list">
                {items.map(item => {
                    const attachEl  = item.sub_elements?.['#commentAttachLink']?.[0];
                    const paths     = attachEl?.download_path;
                    const fileList  = Array.isArray(paths) ? paths : paths ? [paths] : [];
                    const bodyLines = item.text?.split('\n').filter(Boolean) ?? [];

                    return (
                        <div key={item.index} className="ca-card">
                            <div className="ca-card-header">
                                <span className="ca-index">#{item.index}</span>
                                <span className="ca-attach-count">{attachEl?.text}</span>
                                <span className={`ca-status ${attachEl?.downloaded ? 'done' : 'pending'}`}>
                                    {attachEl?.downloaded ? '✅ 완료' : '⬜ 미완료'}
                                </span>
                                {attachEl?.error && (
                                    <span className="ca-error">❌ {attachEl.error}</span>
                                )}
                            </div>
                            <div className="ca-body">
                                <div className="ca-comment">
                                    {bodyLines.map((line, i) => (
                                        <div key={i} className="ca-comment-line">{line}</div>
                                    ))}
                                </div>
                                {fileList.length > 0 && (
                                    <div className="ca-files">
                                        {fileList.map((p, i) => (
                                            <div key={i} className="ca-file">
                                                📄 {p.split(/[\\/]/).pop()}
                                            </div>
                                        ))}
                                    </div>
