...
| `ResultPanel.jsx` | verdict 아이콘/일치도, 매칭 케이스·칩 배지, 원 분석 판정 행(`case_verdict` + `verdict_rationale`), 원 분석 조치 행(`action_summary` + `action_details`), `DefectReferenceControl`(현재 defect를 케이스 참조로 등록/제거 — system명 `"Kona"` 상수), 선정 프로파일, 매칭 패턴, ReactMarkdown 리포트(복사/다운로드), 경고 목록, `MinorityReportSection`(기타 후보 표 — ensemble/first_hit 모드면 빈 상태도 표기) |
...
- **분석 결과 형태**(AA `serialize_result` 산출물): `verdict`("문제"/"문제 아님"/"판정 불가"/"불확실"/"알 수 없음" — 아이콘·클래스 매핑 상수를 `ResultPanel`·`HistoryPage` 양쪽이 보유), `match_level`("높음"/"부분"/"없음" — 일치도 축), `report_md`, `matched_case{case_id,name,chip_tags,references,case_verdict,undetermined_reason,verdict_rationale,action_summary,action_details[],…}`(조치 표기는 AA `case_report.py`가 렌더링해 내려주므로 프론트에서 라벨 매핑을 중복 보유하지 않는다), `match_result{score,matched[],unmatched[]}`, `minority_reports[]`, `winner_profile_names`, `warnings`, `traversal_mode`. `score`는 결함 확률이 아니라 패턴 일치도이므로 "일치도 NN%"로 표기한다 (AA 설계서 §6.3).
