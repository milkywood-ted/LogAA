...
| 결정 | 채택 이유 / 대가 |
| --- | --- |
| chip을 fetch 시 1회 영속 + 조회·분석 시 메모리 보정(`resolve_meta`) | 매핑 YAML이 나중에 추가된 기존 defect도 조회·분석 응답에서 chip이 채워짐(파일 불변). 대가: 레거시 defect는 조회마다 재계산(캐시된 YAML 부분매칭이라 미미) — 파일 영속은 재수집 시 |
