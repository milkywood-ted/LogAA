...
| # | 항목 | 내용 (확신도) |
| --- | --- | --- |
| 1 | 신뢰 경계가 네트워크에 위임 → **검토 후 수용(완화 후)** | 작성 시점 대비 구성 요소 대부분 처리됨(2026-07-16~17): IP 허용목록으로 네트워크 대역 제한(backend §9-2), CORS credentials 정리(backend §9-1), user-logs 허용 루트 경계(backend §9-8), 비밀정보 평문은 검토 후 수용(backend §9-3), frontend도 같은 출처·IP 게이트 뒤로 통합(frontend §9-5). 잔여 미결이던 **사용자 단위 인증·전송 암호화는 사내망·인증서 제약상 도입하지 않기로 확정** — 외부 노출·다중 조직 사용 등 전제가 바뀌는 시점에 재검토 (medium) |
...
| 3 | 배포·기동 절차의 비공식성 → **서비스화 킷 제공 (서버 적용 후 해소)** | 2026-07-17 `deploy/`에 systemd user 모드 유닛 3종 + 설치 가이드 제공 — sudo 불필요, 자동 재시작(`Restart=on-failure`)·기동 순서(AA·puller→backend)·journald 로그. 부팅 자동 기동은 linger 1회 설정(또는 재부팅 후 로그인 1회 규칙). frontend dev 서버 운영은 backend 정적 서빙으로 대체됨(frontend §9-5). **잔여: 서버에서 유닛 설치·구동 검증** (가이드의 검증 체크리스트) (medium) |
