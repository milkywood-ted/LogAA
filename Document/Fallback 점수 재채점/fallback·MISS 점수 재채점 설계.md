...
패턴 매칭 자체는 확정적(boolean)이다 — 로그에 그 패턴이 있냐 없냐. DB 전체 대비 비율이라는 개념 자체가 범주 오류였다: 그 비율은 원래 "한 케이스의 체크리스트 중 얼마나 확인됐나"를 재는 도구인데, DB 전체 패턴 풀은 하나의 체크리스트가 아니라 서로 무관한 여러 가설(케이스)의 집합이기 때문이다. 대신 **"몇 개의 서로 다른 패턴이 매칭됐는가"**를 1차 판단 기준으로 삼는다.

- **매칭 패턴 0개**: 기존과 동일 — "유사문제 없음".
- **매칭 패턴 정확히 1개**: 다른 어떤 신호도 함께 걸리지 않았다는 뜻이므로, 그 패턴이 가리키는 문제일 가능성이 사실상 확정이다. 분모/분자 비율을 고민할 필요 없이 그 패턴(또는 그 패턴이 속한 케이스)을 채택한다.
- **매칭 패턴 2개 이상**: 서로 다른 신호가 동시에 걸렸다는 뜻 — 여러 문제가 섞여 있거나, 여러 후보 중 하나를 골라야 하는 모호한 상황. §5 참고.

### 4-1. orphan 패턴(연결된 케이스 없음)의 처리 — "가상의 단일 패턴 케이스"

패턴이 어느 케이스에도 연결돼 있지 않으면(`find_cases_by_pattern_names`가 빈 목록 반환) §3의 재채점 대상에서 빠진다. 이를 "그 패턴 하나만 담은 가상 케이스"로 간주하면 별도 분기 없이 통합된다 — `weight/weight = 1.0`으로, 소속 케이스가 있는 패턴과 동일한 계산 경로를 탄다. 매칭된 orphan 패턴은 항상 자기 자신 기준으로 100% 확정된다(4번 문단의 "1개 매칭=확정" 원칙과 정확히 일치).
