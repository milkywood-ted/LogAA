...
# num_ctx 에서 미리 빼두는 안전 마진(토큰).
#   - 프롬프트 구조 오버헤드 (역할 헤더, 구분선, 형식 지시문 등) ~ 500~1000 토큰
#   - 출력 버퍼 예약 (LLM 의 응답 토큰 공간) ~ 2000~3000 토큰
#     리포트 출력은 일반적으로 1000~2500 토큰이며 Stage 6 Reflection 도 비슷한 수준.
#   - 토큰 추정 자체의 오차 흡수 (CHARS_PER_TOKEN 보수적 추정의 역방향 손실)
# 모델별로 적정값이 달라질 수 있으나(특히 짧은 컨텍스트 모델), 현재는 단일 보수값 유지.
# 실제 컨텍스트 초과로 잘림이 빈번해지면 모델별 설정으로 분리 검토.
OVERHEAD_TOKENS: int = 4_000
