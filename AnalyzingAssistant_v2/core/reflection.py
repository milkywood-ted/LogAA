...
3. 판정 일관성: 점수({score:.0%})는 **패턴 일치도**이며 결함 확률이 아닙니다.
   판정({verdict})은 일치도와 그 케이스의 원 분석 판정을 함께 반영한 결과이므로,
   일치도가 높은데 판정이 "문제 아님"·"판정 불가"인 것은 모순이 아니라 정상입니다.
   판정 레이블 자체는 어떤 경우에도 변경하지 마세요.
   근거 설명이 판정과 어긋날 때만 설명을 수정하세요.
