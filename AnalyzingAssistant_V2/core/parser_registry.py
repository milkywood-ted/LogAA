...
from core.config import parsers_config
...
def _build(raw: dict) -> tuple[list[dict], dict[str, tuple]]:
    """raw dict 에서 정규식 컴파일 + PARSER_DEFS/PARSER_MAP 을 구축한다."""
    defs: list[dict] = []
    pmap: dict[str, tuple] = {}
...
def _load(path: Path | None = None) -> tuple[list[dict], dict[str, tuple]]:
    """yaml 로딩은 parsers_config 에 위임하고, 도메인 로직(정규식 컴파일)은 여기서 수행."""
    raw = parsers_config.load() if path is None else parsers_config.load(path)
    return _build(raw)
...
class ParserRegistry:
    ...
    @classmethod
    def from_yaml(cls, path: Path | None = None) -> "ParserRegistry":
        defs, _ = _load(path)
