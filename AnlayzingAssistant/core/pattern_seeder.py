...
PATTERNS_YAML: Path = Path(__file__).parent.parent / "config" / "patterns" / "default_patterns.yaml"
...
def load_yaml(yaml_path: Path = PATTERNS_YAML) -> list[AnyPattern]:
    """
...
def _insert_one(conn, p: AnyPattern, name_to_id: dict[str, int]) -> int:
