...
from core.parser_registry import PARSER_MAP as _PARSER_MAP_REGISTRY, DEFAULT_ACTIVE_PARSERS
...
class LogRefiner:
    ...
    def refine(self, raw_log: str, config: RefineConfig) -> list[LogLine]:
        """
        ...
        lines = self._apply_window(lines, config)                          # 1-3
        return lines
    ...
    # config/log_parsers.yaml 에서 로드된 파서 맵
    _PARSER_MAP = _PARSER_MAP_REGISTRY
    ...
    def _parse_and_filter(self, raw_log: str, active_parsers: list[str]) -> list[LogLine]:
        ...
        """
        parsers = [
            self._PARSER_MAP[name]
            for name in active_parsers
            if name in self._PARSER_MAP
        ]
    ...
    def _apply_window(self, lines: list[LogLine], config: RefineConfig) -> list[LogLine]:
        """
